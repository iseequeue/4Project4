...
void Calculator::calculate(sf::Vector2f center, float width, float height)
{
    ...
    auto function = [=](std::size_t id)
    {
		...
		std::complex < float > c(0.28, 0.0113);
		...
		for (auto y = first_y; y < last_y; ++y)
		{
			...
			{
				std::complex < float > z(center.x - width / 2.0f + dx * x, center.y - height / 2.0f + dy * y);
				
			
				auto iteration = 0U;

				for (; iteration < m_max_iterations && std::norm(z) < 4; ++iteration)
				{
					z = z*z+c;
				}
